...
function getroomnetty_blance(roomid, redis_local)
...
if (tonumber(roomid) > 0) then
    ...
    --get netty and set netty_room
    if not ok then
        ngx.log(ngx.ERR, "connect area redis error : ", redis_blance_host_ip.." "..redis_blance_host_port)
        ngx.exit(503)
        return
    end

    local redis_ip = getroomnetty_blance(roomid, red_blance_global)
    ngx.log(ngx.ERR, "get already netty ip :", redis_ip)

    --get the netty for exist roomid ok
    if isnull(redis_ip) ~= 1 then
        ngx.log(ngx.ERR, "already redis_ip :", redis_ip)
