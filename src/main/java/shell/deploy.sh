...
#删除.run文件，部署完毕
rm -rf ${compile_dir}/.run;

echo "##############################################################"
echo -e "                   开始部署AB-Server                         "
echo "##############################################################"
ps -ef  | grep "AB-dcserver" | grep -v grep | awk  '{print $2}' | xargs kill -9
nohup java -Xmx1024m -Xss256k -jar /opt/jenkins/compile/AB-dcserver/ab-dc-registry-server/target/registry-server-1.0.0-SNAPSHOT.jar --spring.config.location=application.properties  --server.port=8761 &
#echo `$?`
#if [ $? != 0]; then
#    echo "启动注册中心返回非0结果"
#fi
...
echo "在`ps -ef | grep registry-server | awk  '{print $2}'` 端口启动了注册服务"
nohup java -Xmx1024m -Xss256k -jar /opt/jenkins/compile/AB-dcserver/ab-dc-registry-server/target/registry-server-1.0.0-SNAPSHOT.jar --spring.config.location=application.properties  --server.port=8761 &
nohup java -Xmx1024m -Xss256k -jar /opt/jenkins/compile/AB-dcserver/ab-dc-gateway/target/ab-dc-gateway-1.0.0-SNAPSHOT.jar --spring.config.location=application.properties  --server.port=9090 &
#echo `$?`
#if [ $? != 0]; then
#    echo "启动网关中心返回非0结果"
#fi

echo "在`ps -ef | grep gateway | awk  '{print $2}'` 端口启动了网关服务"
...
#第一个hero的端口是8091
#cp /opt/jenkins/compile/AB-dcserver/ab-dc-hero/hero-api/target/ab-dc-hero-api-1.0.0-SNAPSHOT.war /opt/tomcat/hero2/webapps/ROOT.war && sh /opt/tomcat/hero2/bin/startup.sh#!/usr/bin/env bash